...
"""Feetech auto-calibration and WritePosEx-related logic (mechanical range measurement,
default range, single-instruction position+speed+acceleration writes)."""
...
"""Exception types servo communication may raise: RuntimeError(Overload) and ConnectionError(no status packet)."""

logger = logging.getLogger(__name__)


class FeetechCalibrationMixin:
    """Provides automatic mechanical-range measurement, default range, write_pos_ex_and_wait /
    wait_until_stopped."""

    # Single instruction writes Acceleration(41) + Goal_Position(42-43) + Goal_Time(44-45)
    # + Goal_Velocity(46-47), 7 bytes total (matches STServo WritePosEx).
    _POS_EX_START_ADDR = 41
    _POS_EX_LEN = 7
    # Min_Position_Limit(9,2) + Max_Position_Limit(11,2): 4 contiguous bytes, writable in one instruction.
    _POS_LIMITS_START_ADDR = 9
    _POS_LIMITS_LEN = 4

    def get_default_range(self, motor: NameOrID) -> tuple[int, int]:
        """Return the default (range_min, range_max) used during auto-calibration for the given motor.

        If the motor name is in calibration_defaults.SO_STS_DEFAULT_RANGES, the preset value is used;
        otherwise the full range under that model's resolution (0, max_res) is used.
        """
    ...
    def _wait_for_stall(
        ...
        *,
        ...
        """Poll for stall / limit detection and return a stop-reason string.

        First checks the AND condition (velocity near zero + position stable + Moving=0); if met, returns
        immediately. Otherwise checks Status register BIT5 (overload) or comm exceptions; declares stall
        after stall_confirm_samples consecutive hits.
        """
        stall_count = 0
        stable_count = 0
            ...
            if vel_ok and pos_ok and moving_ok:
                ...
                if stable_count >= stall_confirm_samples:
                    return (
                        ...
                        "velocity near zero + position stable + Moving=0"
                    )
            else:
                stable_count = 0
            prev_position = pos

            # Next: Status BIT5 overload
            if status & 0x20:
                stall_count += 1
                if stall_count >= stall_confirm_samples:
    ...
    def _wait_for_stall_multi(
        ...
        *,
        ...
        """Multi-servo stall polling: monitor multiple servos simultaneously; for whichever stalls first,
        write Goal_Velocity=0 to that servo. Returns once all have stalled or the timeout is reached.

        Returns (reasons, positions): per-servo stop reason and Present_Position when stall was detected.
        """
            ...
            for m in list(still_running):
                try:
                    ...
                    if stall_count[m] >= stall_confirm_samples:
                        reasons[m] = f"stall confirmed ({stall_confirm_samples}x): comm exception"
    ...
    def _prepare_motors_for_range_measure(self, motors: list[str]) -> None:
        ...
        for m in motors:
            ...
            if len(motors) == 1:
                print(f"  [{motor_label(m)}] Operating_Mode={mode} (expected 1, constant-speed mode)")
    ...
    def _compute_mid_and_range_from_limits(
        ...
        *,
        ...
        """From CW/CCW stall positions, back off and compute the physical midpoint and range.
        Returns (range_min, range_max, mid, raw_min, raw_max, homing_offset).
        If reference_pos is provided (e.g. (Present_Position+Homing_Offset)%FULL_TURN sampled
        during the lift phase), skip back-off and re-read and use it directly to pick the arc."""
        ...
        if reference_pos is not None:
            ...
            print(
                f"  [{motor_label(motor)}] using pre-sampled reference start_pos={start_pos}, skipping limit back-off"
            )
        else:
            ...
            print(
                ...
                f"back-off steps: {steps_back} ({back_deg:.1f}°)"
            )
            ...
            print(
                ...
                f"to reference: present={present_raw}, offset={homing_raw}, actual={start_pos}"
            )
        arc_ccw_to_cw = (pos_cw - pos_ccw) % FULL_TURN
        arc_cw_to_ccw = (pos_ccw - pos_cw) % FULL_TURN
        ...
        print(
            ...
            f"physical mid={mid}, crosses zero={crosses_zero}"
        )
    ...
    def _safe_stop_and_clear_overload(self, motor: str, settle_s: float = 0.5) -> None:
        """Safe stop after stall: write Goal_Velocity=0 -> disable torque -> wait for
        Overload / comm exception to clear."""
        for _ in range(5):
            try:
        ...
        for i in range(retries):
            try:
                ...
                if i < retries - 1:
                    ...
                    continue
                raise RuntimeError(
                    f"safe_write: failed all {retries} attempts {reg}={value} on {motor_name}: {e}"
                ) from e
    ...
    def _write_torque_with_recovery(
        ...
        """Write Torque_Enable, with recovery and retry depending on exception type.

        - RuntimeError (Overload): disable torque first to clear overload state, wait, then retry.
        - ConnectionError (no response): wait, then retry.
        ...
        for attempt in range(retries):
            try:
                ...
                # Overload: disable torque to clear overload, wait, then retry
                if attempt < retries - 1:
                    ...
                    raise RuntimeError(
                        f"_write_torque_with_recovery: failed all {retries} attempts Torque_Enable={value} on {motor}: {e}"
                    ) from e
            except ConnectionError as e:
                # No response: wait, then retry
                if attempt < retries - 1:
                    print(
    ...
    def _clear_and_enable_torque(self, motor: str, settle_s: float = OVERLOAD_SETTLE_TIME) -> None:
        """Clear overload then re-enable torque: disable torque and wait, then re-enable with recovery retry.

        Used when reverse motion is needed after a stall stop; replaces a bare enable_torque call.
        """
        # Disable torque to clear overload state (retry logic already included)
        for _ in range(5):
            try:
                self.write("Torque_Enable", motor, 0)
                break
                ...
                time.sleep(0.1)
        time.sleep(settle_s)
        # Re-enable torque with recovery retry
    ...
    def safe_write_position_limits(
        ...
        *,
        ...
        """Safe write of Min/Max position limits (single instruction) with retries."""
        motor_name = self._get_motors_list(motor)[0]
        for i in range(retries):
            try:
                ...
                if i < retries - 1:
                    ...
                    continue
                raise RuntimeError(
                    f"safe_write_position_limits: failed all {retries} attempts rmin={rmin} rmax={rmax} on {motor_name}: {e}"
                ) from e
    ...
    def safe_disable_all(
        ...
        *,
        ...
        """Safely disable torque on all servos, ignoring comm exceptions."""
    ...
    def go_to_mid(
        ...
        *,
        ...
        """Send the servo to its midpoint (servo mode) and wait until it arrives.
        Returns True if it arrived, False on timeout."""
        motor_name = self._get_motors_list(motor)[0]
        ...
        if not ok:
            logger.warning(
                "%s centering timed out (%.1fs), current position=%s",
    ...
    def measure_ranges_of_motion(
        ...
        *,
        ...
        """Automatically measure a single motor's mechanical limit range (constant-speed mode).

        Only one motor may be passed. Torque (Max_Torque_Limit, Torque_Limit) and acceleration
        (Acceleration) should be initialized prior to calling. This method only uses Goal_Velocity to drive.

        Flow:
        1. Set register 18 BIT4=0 (single-turn angle feedback, 0-4095)
        2. Clear Homing_Offset
        3. Switch to Operating_Mode=1 (constant-speed mode); Goal_Velocity drives forward/reverse rotation
        4. Detect stall via Status register BIT5 to obtain the CW/CCW limits
        5. Not dependent on starting position: back off from the CCW limit along the opposite of the last
           direction by some steps to obtain reference point P.
           Back-off steps = half of min(distance-to-0, distance-to-4095); if the CCW extremum is exactly
           0 or 4095, use the CW extremum instead.

        Returns:
            Six integers (range_min, range_max, mid, raw_min, raw_max, homing_offset):
            - range_min, range_max: extremes in offset-applied space (0~4095, non-zero-crossing); used to
              write Min/Max position limits.
            - mid: measured physical midpoint (raw encoding).
            - raw_min, raw_max: measured extremes (raw encoding; may cross zero).
            - homing_offset: offset value mid - MID_POS, used to write Homing_Offset.
        """
    ...
    def measure_ranges_of_motion_multi(
        ...
        *,
        ...
        """Measure mechanical limits of multiple servos at once: start them simultaneously, poll for
        stalls (whoever stops first gets Goal_Velocity=0), read positions when all are stopped, then run
        per-servo back-off and midpoint calculations.

        ccw_first: if True, the servo goes CCW first then CW; if a dict, specifies per servo
        (motor_name -> True/False).
        reference_positions: if a reference position ((Present_Position+Homing_Offset)%FULL_TURN) is
        provided for a motor, that calibration skips the limit back-off and uses the position directly
        to pick the arc.
        ...
        # Per-servo direction: CCW first or CW first
    ...
    def read_position_limits(self, motor: NameOrID) -> tuple[int, int]:
        """Read Min_Position_Limit and Max_Position_Limit; returns (rmin, rmax)."""
    ...
    def wait_until_stopped(
        ...
        """Poll the Moving register until it is 0 or the timeout elapses
        (matches the STServo read_write example).

        Returns True if stopped (Moving==0), False on timeout or read failure.
        """
        t0 = time.monotonic()
        while time.monotonic() - t0 < timeout_s:
            try:
                ...
                logger.debug("wait_until_stopped: comm exception reading Moving")
    ...
    def write_pos_ex_and_wait(
        ...
        *,
        ...
        """Write position+speed+acceleration in one instruction, then poll Moving until stopped
        (matches the STServo read_write example).

        First ensures servo mode (Operating_Mode=0), then writes Goal_Position/Goal_Velocity/Acceleration,
        then wait_until_stopped.
        Returns True if it arrived, False on timeout or write failure.
        """
        try:
            self.write("Operating_Mode", motor, 0)  # ensure servo mode, otherwise Goal_Position has no effect
            time.sleep(0.05)
            # Write target position, speed, acceleration in one instruction (7 contiguous bytes
            # from register 41); matches STServo WritePosEx.
            id_ = self._get_motor_id(motor)
    ...
    def sync_write_pos_ex(
        ...
        *,
        ...
        """For multiple servos, first RegWritePosEx into the buffer, then RegAction executes them all
        together (matches the STServo reg_write example).
    ...
    def _unfold_move_and_wait(
        ...
        """In servo mode, use WritePosEx to write the target, poll Moving until stopped, then judge
        arrival or stall from position / Status. Treats error within tolerance_deg degrees as success
        (default 5°)."""
        ...
        if status & 0x20:
            self._clear_overload_unfold(motor)
            print(
                f"    [{motor_label(motor)}] stalled: pos={pos} (Status=0x{status:02X} BIT5 overload, "
    ...
    def _clear_overload_unfold(self, motor: str) -> None:
        """Disable torque to clear Overload state, wait for recovery, then re-enable torque
        (used by the unfold logic)."""
        try:
            self.write("Torque_Enable", motor, 0)
    ...
    def unfold_single_joint(
        ...
        """Unfold a single joint: try the forward direction first, then reverse on failure.

        The initialization phase has already configured PID/Acceleration/Operating_Mode=0,
        so in servo mode only Goal_Position needs to be written to drive the servo.

        Returns:
            (success direction, steps): direction is "forward"/"reverse"; on failure (None, 0).
            Steps is the target step count.
        """
        target_steps = int(unfold_angle / 360.0 * FULL_TURN)
        print(f"\n--- Unfolding {motor_label(motor)} ({target_steps} steps ≈ {unfold_angle:.1f}°) ---")

        # Re-zero midpoint (Torque_Enable=128 sets the current position to 2048)
        ...
        time.sleep(0.1)
        # Restore servo mode (128 may have changed the mode state) and enable torque
